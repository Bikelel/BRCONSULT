...
class CustomerPortal(portal.CustomerPortal):
    ...
    def portal_my_prestations(self, page=1, sortby=None, filterby=None, search=None, search_in='content', groupby=None, **kw):
        ...
        searchbar_filters = {
            ...
            'year': {'label': ('Cette année'), 'domain': [('date', '>=', date_utils.start_of(today, 'year')), ('date', '<=', date_utils.end_of(today, 'year'))]},
            ...
            'last_year': {'label': ("L'année dernière year"), 'domain': [('date', '>=', date_utils.start_of(last_year, 'year')), ('date', '<=', date_utils.end_of(last_year, 'year'))]}
        }
        partner = request.env.user.partner_id
